...
Options:\n
  ...
  -U | --uninstall - Uninstall the script and remove related files. Use 'force' as argument for removal of remnants from shell rc, if any, even if command is not installed\n
...
_start() {
    ...
    chmod -f u+w "${INSTALL_PATH}/${COMMAND_NAME}" 2>| /dev/null || :
...
_setup_arguments() {
    ...
    while [ $# -gt 0 ]; do
        case "${1}" in
            ...
            -U | --uninstall)
                UNINSTALL="true"
                [ "${2}" = force ] && FORCE_UNINSTALL="true" && shift
                ;;
    ...
    _check_existing_command() {
        if COMMAND_PATH="$(command -v "${COMMAND_NAME}")"; then
            if SCRIPT_VALUES="$(grep -E "${VALUES_REGEX}|^LATEST_INSTALLED_SHA=\".*\".* # added values|^SELF_SOURCE=\".*\"" "${COMMAND_PATH}" || :)" &&
                eval "${SCRIPT_VALUES} && :" && [ -n "${LATEST_INSTALLED_SHA:+${SELF_SOURCE:-${FORCE_UNINSTALL}}}" ]; then
                [ -n "${FORCE_UNINSTALL}" ] && return 1 || return 0
            else
                ...
                printf "%s\n" "Otherwise uninstall the ${COMMAND_NAME} command manually or using sh ${0} -U force and run this script again."
                exit 1
            fi
        else
            return 1
